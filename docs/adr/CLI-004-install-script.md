...
**Amended:** 2026-07-26 — **the scripts now build from source, not GitHub Releases.** The release channel is empty while Cambrian is under daily development, so a release-download installer can only ever produce nothing. `install.sh` / `install.ps1` clone `cambrian-sh/{cli,core,agent-sdk-python}` into `$CAMBRIAN_HOME/src`, build `cambrian` (bun) + `cambrian-orchestrator` (go, `-X main.version=$(git describe)`), and install to `~/.cambrian/bin`. Consequences of the swap: the asset/`SHA256SUMS` verification step below is **not implemented** (integrity now comes from cloning over HTTPS from the canonical org — a weaker guarantee, revisit when releases exist); the "latest tag" idempotency check is replaced by a `cli=<sha> core=<sha>` build stamp; git + go ≥1.21 + bun ≥1.3 became hard prerequisites; re-running the script is the upgrade path (fetch → reset → rebuild) and it refuses to reset a dirty checkout. Everything else in this ADR (the D7 split, `~/.cambrian` only, no sudo, PATH handling, `/dev/tty` telemetry opt-in, hand-off to `cambrian init`) is unchanged. **Revert to the release path once release CI produces assets.**
**Author:** CLI initiative
**Depends on:** CLI-003 (distribution — superseded for now by the source-build amendment above)
